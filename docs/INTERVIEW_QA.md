...
V2.1 的 20 个训练实例、5 个独立评测实例和候选位置去偏已经完成。下一步先训练一轮 V2.1 SFT，并检查 ranking 模板、筛选后 candidate accuracy 和位置鲁棒性；只有基础比较能力稳定后才重新做 GRPO，最后再考虑换 1.5B。
...
我为同一套实体生成 20 个训练页面实例，每个实例使用全新随机 ID，并对候选列表做循环换位。这样属性和正确答案不变，但正确候选会出现在不同位置。最终同一 template-answer 至少覆盖 4 个位置，任一模板的最大单位置占比只有 26%，因此位置不能再稳定预测答案。

### 22. 数据生成时遇到过什么不明显的问题？

第一版按全局 task index 同时轮转 15 个模板和 20 个实例。因为两者最大公约数是 5，每个模板实际只覆盖 4 个实例。总量和模板计数看起来都正常，所以只做边际统计发现不了。我改成按模板自己的 occurrence index 分配 context，并增加测试，要求每个模板覆盖全部训练实例。这个例子说明数据管线需要检查联合分布，而不只是总数。

### 23. V2.1 的 7000/7000 成功意味着模型已经提升了吗？

不意味着。7000/7000 是规则专家对数据和环境的完整性验证，只能证明任务可解、路径正确、没有非法动作。模型是否提升必须等 V2.1 SFT 后，在独立的 1000-task eval 上与 V2 baseline 比较。
